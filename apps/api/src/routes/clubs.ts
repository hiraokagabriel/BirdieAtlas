...
export async function clubsRoutes(app: FastifyInstance) {
  ...
  // Returns club metadata + current roster with ranking points per discipline
  // ---------------------------------------------------------------------------
  app.get('/clubs/:id/profile', async (request, reply) => {
    ...
    // Plantel atual: afiliações sem endedAt
    ...
    if (!currentAffiliations.length) {
      return { club, roster: [], totalPoints: 0, avgPoints: 0, rankAmongClubs: null }
    }
    ...
    // Ranking entries do tenant do clube
    ...
    // Busca todas as entries dos atletas do plantel
    const entries = rankingIds.length
      ? await db
          ...
          .where(
            and(
              inArray(rankingEntries.rankingId, rankingIds),
              inArray(rankingEntries.athleteId, athleteIds)
            )
          )
      : []

    // Agrega pontos por atleta (soma de todas as disciplinas)
    ...
    // Rank do clube: compara com outros clubes do mesmo tenant
    // Busca todos os atletas de todos os clubes ativos do tenant
    ...
    for (const e of allEntries) {
      allEntriesMap.set(e.athleteId, (allEntriesMap.get(e.athleteId) ?? 0) + e.points)
    }
