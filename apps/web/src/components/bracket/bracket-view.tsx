...
import { Eye, EyeOff, Loader2, Trophy, AlertTriangle, CheckCircle2, Shuffle, Star } from 'lucide-react'
...
interface DrawModeCardProps {
  mode: DrawMode
  selected: boolean
  onSelect: (m: DrawMode) => void
}

function DrawModeCard({ mode, selected, onSelect }: DrawModeCardProps) {
    ...
    >
      ...
      <div>
        <p className="font-semibold text-sm text-center">
          {isRandom ? 'Sorteio Aleatório' : 'Com Cabeças de Chave'}
        </p>
  ...
  const { mutate: generateDraw, isPending: isGenerating } = useMutation({
    mutationFn: async () => {
      ...
      if (!res.ok) {
        ...
        throw new Error(e.error ?? 'Erro ao gerar chaveamento')
      }
      return res.json()
    },
    onSuccess: () => {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {categories.map((cat) => {
          ...
          return (
            <button
              key={cat.id}
              onClick={() => { setActiveCategoryId(cat.id); setGenerateError(null) }}
      ...
      <div className="rounded-xl border border-border bg-card overflow-auto">
        {isLoading ? (
          ...
          // ── Empty state: draw mode selection ──────────────────────────────
